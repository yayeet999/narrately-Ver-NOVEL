...
-- Create trigger for outline data validation
CREATE TRIGGER validate_outline_data
    BEFORE INSERT OR UPDATE ON novels
    FOR EACH ROW
    EXECUTE FUNCTION validate_outline_data();

-- Create function to validate processed metrics
...
-- Create trigger for processed metrics validation
CREATE TRIGGER validate_processed_metrics
    BEFORE INSERT OR UPDATE ON novels
    FOR EACH ROW
    EXECUTE FUNCTION validate_processed_metrics();

-- Create function to cleanup abandoned generations
...
-- Create function to get novel progress
